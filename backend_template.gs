/**
 * ON-THE-GO APPSUITE - MASTER BACKEND v34.0 (Smart Scribe Edition)
...
const CONFIG = {
  ...
  GEMINI_API_KEY: "%%GEMINI_API_KEY%%", // Used for Smart Scribe
...
function processFormEmail(p) {
    try {
        ...
        // Clean the Main Notes if they exist
        ...
        for (const [key, val] of Object.entries(reportData)) {
            ...
            // Clean Text Fields via Smart Scribe
            let displayVal = val;
            if (typeof val === 'string' && val.length > 10 && !val.includes('http')) {
               // Only clean long text fields, avoid numbers/coordinates
...
function generateVisitPdf(rowIndex) {
    ...
    try {
      ...
      headers.forEach((header, i) => {
          ...
          // --- SMART SCRIBE PROCESSING ---
          // Apply to "Notes" and "Visit Report Data" if it looks like text
