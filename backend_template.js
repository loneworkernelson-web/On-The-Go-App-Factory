/**
 * OTG APPSUITE - MASTER BACKEND v52.0 (Gatekeeper Edition)
 * Security Upgrade: Checks 'Staff' tab for authorization.
 */
...
function isAuthorized(workerName) {
  ...
  // BASIC MODE: If no 'Staff' tab exists, allow everyone (Backward Compatibility)
  if (!sheet) return true; 

  // SECURE MODE: Check the list
  const data = sheet.getDataRange().getValues();
  // Assume Row 1 is Headers. Check Col A (Name) and Col C (Status)
  for (let i = 1; i < data.length; i++) {
    const rowName = String(data[i][0]).trim().toLowerCase();
    const targetName = String(workerName).trim().toLowerCase();
    
    if (rowName === targetName) {
       // Optional: Check for "Inactive" status in Col C
...
function doGet(e) {
  ...
  if(e.parameter.action === 'sync') {
      ...
      if (!isAuthorized(worker)) {
          return ContentService.createTextOutput(JSON.stringify({
              status: "error", 
              message: "ACCESS DENIED: Your name is not in the authorized Staff list."
  ...
  if(e.parameter.callback){
    ...
    return ContentService.createTextOutput(e.parameter.callback+"("+JSON.stringify({ workers: rows, server_time: new Date().toISOString(), watchdog_last_run: lastRun })+")").setMimeType(ContentService.MimeType.JAVASCRIPT);
  }
...
function doPost(e) {
  ...
  try {
    ...
    // GATEKEEPER CHECK (Block unauthorized uploads)
    if (!isAuthorized(p['Worker Name'])) {
         return ContentService.createTextOutput(JSON.stringify({status: "error", message: "Unauthorized Worker"}));
    }
    ...
    ['Photo 1', 'Photo 2', 'Photo 3', 'Photo 4', 'Signature'].forEach(key => {
        if(p[key] && p[key].length > 100) {
             ...
             assets[key] = saveImageToDrive(p[key], `${safeWorker}_${key.replace(' ', '')}_${Date.now()}.${ext}`);
        } else { assets[key] = ""; }
    });
    ...
    if (p['Template Name'] && p['Visit Report Data'] && p['Visit Report Data'].length > 5) processFormEmail(p, assets);
...
function processFormEmail(p, assets) {
    try {
        ...
        let html = `<div style="font-family: sans-serif; max-width: 600px; border: 1px solid #ddd; padding: 20px;"><h2 style="color: #2563eb;">${p['Template Name']}</h2><p><strong>Submitted by:</strong> ${worker}<br><strong>Location:</strong> ${loc}<br><strong>Time:</strong> ${new Date().toLocaleString()}</p><div style="background:#f8fafc; padding:10px; margin-bottom:15px; border-left:4px solid #3b82f6;"><strong>Notes:</strong> ${displayNotes}</div><hr><table style="width:100%; border-collapse: collapse;">`;
