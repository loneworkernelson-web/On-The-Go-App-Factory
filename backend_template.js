...
function dataURItoBlob(dataURI) {
    // PATCH 2: ROBUST PHOTO HANDLING
    // Handles dataURI with or without 'base64,' prefix
    try {
...
function checkOverdueVisits() {
    // PATCH 3: DEAD MAN'S SWITCH RELIABILITY
    // Added try/catch block so one error doesn't kill the whole safety sweep
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName('Visits');
    if(!sheet) return;
    const data = sheet.getDataRange().getValues();
...
function saveImage(b64, workerName, isSignature) {
    ...
    try {
        // PATCH: Use dataURItoBlob for robust decoding
        ...
        blob.setName(fileName); // Rename blob before saving
...
function smartScribe(data, type, notes) {
    // PATCH 4: AI Model Version Check & Error Safety
    ...
    try {
        // Model verified: gemini-1.5-flash is current and cost-effective
        ...
        if (json.candidates && json.candidates.length > 0) {
            ...
            console.log("AI Scribe returned no candidates. Using original.");
            return notes;
        }
    } catch (e) { 
        console.error("AI Scribe Error: " + e.toString());
        return notes; 
    }
}
...
function sendResponse(e, data) {
    ...
    // Otherwise return standard JSON (Worker App)
