/**
 * OTG APPSUITE - MASTER BACKEND v75.0 (GOLDEN)
 * Features: Staged Escalation, Timezones, Global Forms, Resolution Alerts.
 */
...
function doGet(e) {
  try {
      ...
      if(p.test) {
         if(p.key === CONFIG.MASTER_KEY || p.key === CONFIG.WORKER_KEY) return sendJSON({status:"success", version: "v75.0"});
      ...
      return sendJSON({status: "online", version: "v75.0"});
...
// ... (Rest of Helper Functions: handleSync, handleGeocode, etc. remain unchanged from v74.3) ...
function handleSync(p) { if(p.key !== CONFIG.MASTER_KEY && p.key !== CONFIG.WORKER_KEY) return sendJSON({status:"error", message:"Auth Failed"}); const worker = p.worker; const deviceId = p.deviceId; const auth = checkAccess(worker, deviceId, true); if (!auth.allowed) return sendJSON({ status: "error", message: "ACCESS DENIED: " + auth.msg }); const ss = SpreadsheetApp.getActiveSpreadsheet(); const forms = getFormsFromSheet(ss); const cachedTemplates = {}; forms.forEach(f => cachedTemplates[f.name] = f.questions); const sSheet = ss.getSheetByName('Sites'); const sData = sSheet ? sSheet.getDataRange().getValues() : []; const sites = []; for(let i=1; i<sData.length; i++) { const row = sData[i]; if(row.length < 1) continue; const assign = row[0]; if(assign.includes(worker) || assign === 'ALL') { sites.push({ template: row[1], company: row[2], siteName: row[3], address: row[4], contactName: row[5], contactPhone: row[6], contactEmail: row[7], notes: row[8] }); } } return sendJSON({ sites: sites, forms: forms, cachedTemplates: cachedTemplates, meta: auth.meta }); }
function handleGetForms() { const ss = SpreadsheetApp.getActiveSpreadsheet(); const forms = getFormsFromSheet(ss); return sendJSON(forms); }
function getFormsFromSheet(ss) { const sheet = ss.getSheetByName('Templates'); if(!sheet) return []; const data = sheet.getDataRange().getValues(); const forms = []; for (let i = 1; i < data.length; i++) { const row = data[i]; if(row[0] === "") continue; const questions = parseQuestions(row); forms.push({ type: row[0], name: row[1], questions: questions }); } return forms; }
function handleGeocode(p) { if(!CONFIG.ORS_API_KEY) return sendJSON({address: "No API Key"}); try { const url = `https://api.openrouteservice.org/geocode/reverse?api_key=${CONFIG.ORS_API_KEY}&point.lat=${p.lat}&point.lon=${p.lon}&size=1`; const r = UrlFetchApp.fetch(url); const j = JSON.parse(r.getContentText()); const addr = j.features[0].properties.label; return sendJSON({address: addr}); } catch(e) { return sendJSON({address: "Location lookup failed"}); } }
function handleResolution(p, ss) { const sheet = ss.getSheetByName('Visits'); const data = sheet.getDataRange().getValues(); const targetWorker = p['Worker Name']; let email = "", phone = ""; for (let i = data.length - 1; i >= 1; i--) { if (data[i][2] === targetWorker) { email = data[i][6]; phone = data[i][5]; break; } } const row = [ new Date(), Utilities.formatDate(new Date(), CONFIG.TIMEZONE, "yyyy-MM-dd"), targetWorker, "", "", "", email, "", "", "", "SAFE - MANUALLY CLEARED", p['Notes'], "HQ Resolution", "", "", new Date().toISOString() ]; sheet.appendRow(row); const resolveData = {...p, 'Emergency Contact Email': email, 'Emergency Contact Number': phone}; sendAlert(resolveData, true, 'all'); return sendJSON({status: "resolved"}); }
function checkAccess(workerName, deviceId, isReadOnly) { if (!workerName) return { allowed: false, msg: "Name missing" }; const ss = SpreadsheetApp.getActiveSpreadsheet(); const sheet = ss.getSheetByName('Staff'); if (!sheet) return { allowed: true, meta: {} }; const data = sheet.getDataRange().getValues(); for (let i = 1; i < data.length; i++) { if(!data[i] || !data[i][0]) continue; const rowName = String(data[i][0]).trim().toLowerCase(); if (rowName === String(workerName).trim().toLowerCase()) { if (data[i][2] && String(data[i][2]).toLowerCase().includes('inactive')) return { allowed: false, msg: "Account Disabled" }; const registeredId = String(data[i][4] || ""); if (registeredId === "" || registeredId === "undefined") { if(deviceId && !isReadOnly) { try { sheet.getRange(i + 1, 5).setValue(deviceId); } catch(e) {} } return { allowed: true, meta: getRowMeta(data[i]) }; } else { if (registeredId === deviceId) return { allowed: true, meta: getRowMeta(data[i]) }; else return { allowed: false, msg: "Unauthorized Device. Contact Admin to reset." }; } } } return { allowed: false, msg: "Name not found in Staff list." }; }
...
function handleMonitorPoll(callback) { const ss = SpreadsheetApp.getActiveSpreadsheet(); const t = ss.getSheetByName('Visits'); if(!t) return ContentService.createTextOutput(callback+"("+JSON.stringify({status:"error"})+")").setMimeType(ContentService.MimeType.JAVASCRIPT); const r = t.getDataRange().getValues(); const headers = r.shift(); const st = ss.getSheetByName('Staff'); const stD = st ? st.getDataRange().getValues() : []; const wofMap = {}; if(stD.length > 1) { for(let i=1; i<stD.length; i++) { if(stD[i] && stD[i][0]) wofMap[String(stD[i][0]).toLowerCase()] = stD[i][6] || ""; } } const rows = r.map(e => { let obj = {}; headers.forEach((h, idx) => obj[h] = e[idx]); const wName = obj['Worker Name'] ? String(obj['Worker Name']).toLowerCase() : ""; obj.WOFExpiry = wofMap[wName] || ""; return obj; }); return ContentService.createTextOutput(callback+"("+JSON.stringify({ workers: rows, server_time: new Date().toISOString(), escalation_limit: CONFIG.ESCALATION_MINUTES })+")").setMimeType(ContentService.MimeType.JAVASCRIPT); }
...
function runAllLongitudinalReports() { const ss = SpreadsheetApp.getActiveSpreadsheet(); let allData = []; const sheet = ss.getSheetByName('Visits'); if (sheet) { const vData = sheet.getDataRange().getValues(); if(vData.length > 1) allData = allData.concat(vData.slice(1)); } const archive = ss.getSheetByName('Archive'); if (archive) { const aData = archive.getDataRange().getValues(); if(aData.length > 1) allData = allData.concat(aData.slice(1)); } if (allData.length === 0) return; const dateStr = Utilities.formatDate(new Date(), CONFIG.TIMEZONE, "yyyy-MM"); const name = `Longitudinal Report - ${dateStr} - ${CONFIG.ORG_NAME}`; let reportFile; const files = DriveApp.getFilesByName(name); if (files.hasNext()) reportFile = files.next(); else reportFile = DriveApp.getFileById(SpreadsheetApp.create(name).getId()); const reportSS = SpreadsheetApp.open(reportFile); let sheetAct = reportSS.getSheetByName('Worker Activity'); if (sheetAct) sheetAct.clear(); else sheetAct = reportSS.insertSheet('Worker Activity'); sheetAct.appendRow(["Worker Name", "Total Visits", "Alerts Triggered"]); sheetAct.getRange(1,1,1,3).setFontWeight("bold").setBackground("#dbeafe"); const stats = {}; for (let i = 0; i < allData.length; i++) { const worker = allData[i][2]; const status = allData[i][10]; if (!stats[worker]) stats[worker] = { visits: 0, alerts: 0 }; stats[worker].visits++; if (status.includes("EMERGENCY") || status.includes("OVERDUE")) stats[worker].alerts++; } const actRows = Object.keys(stats).map(w => [w, stats[w].visits, stats[w].alerts]); if (actRows.length > 0) sheetAct.getRange(2, 1, actRows.length, 3).setValues(actRows); let sheetTrav = reportSS.getSheetByName('Travel Stats'); if (sheetTrav) sheetTrav.clear(); else sheetTrav = reportSS.insertSheet('Travel Stats'); sheetTrav.appendRow(["Worker Name", "Total Distance (km)", "Trips"]); sheetTrav.getRange(1,1,1,3).setFontWeight("bold").setBackground("#dcfce7"); const tStats = {}; for (let i = 0; i < allData.length; i++) { const worker = allData[i][2]; const dist = parseFloat(allData[i][18]) || 0; if (!tStats[worker]) tStats[worker] = { km: 0, trips: 0 }; if (dist > 0) { tStats[worker].km += dist; tStats[worker].trips++; } } const travRows = Object.keys(tStats).map(w => [w, tStats[w].km.toFixed(2), tStats[w].trips]); if (travRows.length > 0) sheetTrav.getRange(2, 1, travRows.length, 3).setValues(travRows); MailApp.sendEmail({ to: Session.getEffectiveUser().getEmail(), subject: `Report: ${name}`, htmlBody: `<a href="${reportSS.getUrl()}">View Report</a>` }); }
